...
WSL1 had a proxy I think, It worked out of the box and for some reason they removed that from WSL2. More info here on (https://docs.microsoft.com/en-us/windows/wsl/compare-versions#accessing-a-wsl-2-distribution-from-your-local-area-network-lan)[Microsoft Page] 
...
Now, after restarting windows - the config is being reset because the WSL IP is always being changed after restart. So after digging a bit I found a way to make this "permanent" and that is by running task scheduler and calling this script every time the PC starts - (github link)[https://github.com/microsoft/WSL/issues/4150#issuecomment-504209723]
