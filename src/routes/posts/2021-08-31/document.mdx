...
Now, what we have to do is to do a `git pull` or `git push` in the repository we have where we will get the git prompt to enter the username and password. However, we will cancel this and instead we will write `git config credential.helper cache`
