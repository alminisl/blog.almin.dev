...
This code snippet is only for "CTRL + SHIFT + M" (mute / unmute)
...
And [RobotJs](https://github.com/octalmage/robotjs) is being used to setup the keyboard shortcuts with the `down` and `up` events which are triggered when pressing the button. 
